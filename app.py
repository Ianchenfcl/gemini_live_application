...
async def summarize_transcript(req: SummarizeRequest):
    """將語音對話逐字稿智慧整理為筆記"""
    try:
        genai_client = genai.Client(api_key=req.api_key)
        
        system_instruction = """請扮演專業的筆記整理助手。以下是使用者與 AI 之間的語音對話逐字稿。
請整理出一份專業的「智慧筆記」，必須包含：
1. **核心問題簡述**：簡單概述使用者的主要需求與問題。
2. **重點摘要分析**：分點列出最重要的核心洞見與解法。
3. **具體行動建議**：條列使用者接下來可以直接執行的下一步。

請遵守以下規則：
- 直接輸出內容，不要回應「好的」等無意義前綴。
- 使用繁體中文。
- 使用 Markdown 語法讓排版美觀。"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        model = setup_json.get("model", MODEL)
        video_mode = setup_json.get("video_mode", "none")
        voice_name = setup_json.get("voice_name", DEFAULT_VOICE)
        enable_translation = setup_json.get("enable_translation", False)
        source_language = setup_json.get("source_language", "auto")
        target_language = setup_json.get("target_language", "en")
        ...
        if "translate" in model:
            # 特化版翻譯模型僅支援純語音，不支援視訊解析度與長文脈壓縮等一般對話功能
            if "media_resolution" in config_kwargs:
                del config_kwargs["media_resolution"]
            if "context_window_compression" in config_kwargs:
                del config_kwargs["context_window_compression"]
            
        if enable_translation:
            # 使用 System Instruction 達成即時雙向/單向翻譯 (支援所有 Live 模型)
            lang_map = {
                "zh-TW": "繁體中文", "en": "英文", "ja": "日文", 
                "ko": "韓文", "es": "西班牙文", "fr": "法文", "auto": "使用者說的任何語言"
            }
            bcp47_map = {
                "zh-TW": "cmn-TW", "en": "en-US", "ja": "ja-JP", 
                "ko": "ko-KR", "es": "es-ES", "fr": "fr-FR"
            }
            src_str = lang_map.get(source_language, source_language)
            tgt_str = lang_map.get(target_language, target_language)
            
            translate_instruction = (
                f"你現在是一個專業的同步口譯員。請將接收到的語音（{src_str}）"
                f"即時翻譯成 {tgt_str}，並一律使用 {tgt_str} 語音輸出結果。"
                f"不需要加上任何前綴、問候語或解釋，只需要直接說出翻譯後的內容。"
            )
            config_kwargs["system_instruction"] = translate_instruction
            
            # 強制設定輸出語音的語言代碼，避免某些聲音模型卡死
            if target_language in bcp47_map:
                config_kwargs["speech_config"] = types.SpeechConfig(
                    voice_config=types.VoiceConfig(
                        prebuilt_voice_config=types.PrebuiltVoiceConfig(voice_name=voice_name)
                    ),
                    language_code=bcp47_map[target_language]
                )
        ...
        async with client.aio.live.connect(model=model, config=config) as session:
            ...
            async def receive_from_gemini():
                try:
                    ...
                    error_msg = str(e)
                    logger.error(f"接收 Gemini 回應時出錯: {error_msg}")
                    # 如果是 1008 Session 逾時錯誤，不要發送 error 讓前端強制斷線，
                    # 而是安靜地讓連線中斷，觸發前端的 triggerAutoReconnect() 進行無縫重連。
                    if "1008" not in error_msg:
                        await websocket.send_json({"type": "error", "message": f"接收 Gemini 回應失敗: {error_msg}"})
                ...
                try:
                    while True:
                        ...
                        if item_type == "audio":
                            ...
                            # 使用新版 session.send 傳送對話文字並觸發回應 (避免 1007 錯誤)
                            await session.send(input=item_data, end_of_turn=True)
        ...
        error_msg = str(e)
        logger.error(f"伺服器運作錯誤: {error_msg}")
        traceback.print_exc()
        try:
            if "1008" not in error_msg:
                await websocket.send_json({"type": "error", "message": f"連線錯誤: {error_msg}"})
        except:
            pass
    finally:
        # 清除所有背景任務
