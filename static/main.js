...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const translateConfigArea = document.getElementById("translateConfigArea");
    const enableTranslation = document.getElementById("enableTranslation");
    
    if (enableTranslation && translateConfigArea) {
        enableTranslation.addEventListener("change", () => {
            translateConfigArea.style.display = enableTranslation.checked ? "grid" : "none";
        });
        
        if (modelSelect) {
            modelSelect.addEventListener("change", () => {
                if (modelSelect.value.includes("translate")) {
                    enableTranslation.checked = true;
                    translateConfigArea.style.display = "grid";
                }
            });
            if (modelSelect.value.includes("translate")) {
                enableTranslation.checked = true;
    ...
    // 音訊來源選擇改變時，更新檔案上傳區塊顯示
    const audioSourceSelect = document.getElementById("audioSource");
    const fileInputGroup = document.getElementById("fileInputGroup");
    if (audioSourceSelect && fileInputGroup) {
        audioSourceSelect.addEventListener("change", () => {
            fileInputGroup.style.display = audioSourceSelect.value === "file" ? "block" : "none";
        });
    }
    ...
    if (localMediaFile && localVideoPlayer) {
        localMediaFile.addEventListener("change", (e) => {
...
async function connect(isReconnect = false) {
    ...
    try {
        ...
        // B. 請求音訊授權並建立串流
        if (!isReconnect && micStream) {
            stopMicInput(); // 確保使用者切換音訊來源時，能重新擷取新的來源
        }
        if (!micStream) {
            await startMicInput();
        }
        ...
        ws.onopen = () => {
            ...
            const modelSelect = document.getElementById("modelSelect");
            const srcLangSelect = document.getElementById("sourceLanguage");
            const tgtLangSelect = document.getElementById("targetLanguage");
            const enableTranslation = document.getElementById("enableTranslation");
            
            const setupMsg = {
                type: "setup",
                api_key: apiKey,
                model: modelSelect ? modelSelect.value : "models/gemini-3.1-flash-live-preview",
                video_mode: voiceModeSelect.value,
                voice_name: voiceSelect.value,
                enable_translation: enableTranslation ? enableTranslation.checked : false,
                source_language: srcLangSelect ? srcLangSelect.value : "auto",
                target_language: tgtLangSelect ? tgtLangSelect.value : "en",
...
async function startMicInput() {
    const audioSource = document.getElementById("audioSource")?.value || "mic";
    
    try {
        if (audioSource === "file") {
            const localVideoPlayer = document.getElementById("localVideoPlayer");
            if (!localVideoPlayer || !localVideoPlayer.src) {
                throw new Error("請先選擇本機音訊/影片檔案！");
            }
            micStream = null;
        } else if (audioSource === "screen") {
            const voiceMode = voiceModeSelect ? voiceModeSelect.value : "none";
            sharedScreenStream = await navigator.mediaDevices.getDisplayMedia({
                audio: true,
                video: voiceMode === "screen" ? { width: { ideal: 1024 }, height: { ideal: 768 }, frameRate: { ideal: 5 } } : true
            });
            
            if (voiceMode !== "screen") {
                sharedScreenStream.getVideoTracks().forEach(track => track.stop());
            }

            if (sharedScreenStream.getAudioTracks().length === 0) {
                throw new Error("未能獲取螢幕音訊，請確認分享時有勾選「分享分頁音訊」！");
            }
            micStream = sharedScreenStream;
            addLogMessage("已成功擷取螢幕分享音訊", "info");
        } else {
            // 請求麥克風權限
        ...
        addLogMessage(`音訊擷取失敗: ${e.message}`, "error");
        throw e;
    }
    ...
    if (audioSource === "file") {
        const localVideoPlayer = document.getElementById("localVideoPlayer");
        // 為了避免重複建立 MediaElementSource 發生錯誤
        if (!localVideoPlayer.mediaElementSource) {
            localVideoPlayer.mediaElementSource = audioCtx.createMediaElementSource(localVideoPlayer);
        }
        micSource = localVideoPlayer.mediaElementSource;
        
        // 將音訊連接到 Destination 但音量極低，讓使用者能隱約聽到原始聲音以對照翻譯進度
        const monitorGain = audioCtx.createGain();
        monitorGain.gain.value = 0.05; // 5% 音量
        micSource.connect(monitorGain);
        monitorGain.connect(audioCtx.destination);
        
        addLogMessage("已載入本機媒體檔案，原始音訊音量降為 5% (可手動操作播放器)", "info");
    } else {
    ...
    scriptNode.onaudioprocess = (e) => {
        ...
        // 增加 1.0 秒的緩衝時間，確保句子間的微小停頓不會讓麥克風瞬間開啟而引發打斷
        const timeSinceSpeaking = audioCtx ? (audioCtx.currentTime - nextStartTime) : 999;
        const isGeminiSpeaking = nextStartTime > 0 && timeSinceSpeaking < 1.0;
        
        // 翻譯防打斷功能：若開啟此選項且 Gemini 正在說話 (或剛說完的 1 秒內)，則強制送出靜音封包 (半雙工模式)
        const disableInterruptionCb = document.getElementById("disableInterruption");
        const enableTranslationCb = document.getElementById("enableTranslation");
        if (isGeminiSpeaking && disableInterruptionCb && disableInterruptionCb.checked && enableTranslationCb && enableTranslationCb.checked) {
            const silentBuffer = new Int16Array(inputData.length); // 預設全為 0
            ws.send(silentBuffer.buffer);
            return;
        }
...
async function startVideoInput(mode) {
    ...
    videoPreviewWrapper.style.display = mode === "file" ? "none" : "block";
    let modeText = "鏡頭";
    if (mode === "screen") modeText = "螢幕分享";
    if (mode === "file") modeText = "本機影片";
    addLogMessage(`正在啟用視訊串流... 模式: ${modeText}`, "info");

    let sourceVideo = localVideo;
    ...
    try {
        if (mode === "camera") {
            ...
            if (sharedScreenStream && sharedScreenStream.getVideoTracks().length > 0) {
                videoStream = sharedScreenStream;
            } else {
            ...
            sourceVideo = document.getElementById("localVideoPlayer");
            if (!sourceVideo || !sourceVideo.src) {
                throw new Error("影像模式選擇了「本機影片」，但尚未選擇任何檔案！");
            }
            videoStream = null;
        }
        ...
        videoInterval = setInterval(() => {
            ...
            // 繪製目前的視訊影格
            hCtx.drawImage(sourceVideo, 0, 0, w, h);
...
function autoSaveNote() {
    const autoSaveCheckbox = document.getElementById('auto-save-note');
    if (!autoSaveCheckbox || !autoSaveCheckbox.checked) return;
    
    if (!conversationHistory || conversationHistory.length === 0) return;

    let noteContent = "[通話結束時間] " + new Date().toLocaleString() + "\n";
    noteContent += "[逐字稿紀錄]\n====================================\n\n";
    
    conversationHistory.forEach(turn => {
        const speaker = turn.role === "user" ? "👤 您" : "✨ Gemini";
        noteContent += `${speaker}：\n${turn.text}\n\n`;
    });

    const noteTitle = `語音對話紀錄 - ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`;
    const newNote = {
    ...
    let notes = JSON.parse(localStorage.getItem("live_stream_notes") || "[]");
    notes.unshift(newNote); // 加到最前面
    localStorage.setItem("live_stream_notes", JSON.stringify(notes));
...
function renderNotes() {
    ...
    if (notes.length === 0) {
        notesContainer.innerHTML = `
            <div class="system-message">
                ...
                <span>目前尚無筆記。通話結束後，如果啟用了「自動儲存逐字稿」，這裡將會出現筆記。</span>
            </div>
        `;
        return;
    }
    ...
    notes.forEach(note => {
        ...
        noteDiv.style.cssText = "background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 12px; margin-bottom: 10px;";
        
        const titleEl = document.createElement("h4");
        titleEl.textContent = note.title;
        titleEl.style.cssText = "margin: 0 0 8px 0; font-size: 14px; color: var(--color-teal);";
        
        const contentEl = document.createElement("pre");
        contentEl.textContent = note.content;
        contentEl.style.cssText = "font-size: 12px; color: var(--text-muted); max-height: 100px; overflow-y: auto; white-space: pre-wrap; background: rgba(0,0,0,0.2); padding: 8px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); margin: 0 0 8px 0;";
        
        noteDiv.appendChild(titleEl);
        noteDiv.appendChild(contentEl);

        // 如果已有 summary 則顯示，否則顯示按鈕
        if (note.summary) {
            const summaryDiv = document.createElement("div");
            summaryDiv.style.cssText = "background: rgba(43, 218, 179, 0.1); border-left: 3px solid var(--color-teal); padding: 10px; border-radius: 0 4px 4px 0; margin-top: 10px; font-size: 13px;";
            summaryDiv.innerHTML = `<div style="font-weight: bold; color: var(--color-teal); margin-bottom: 5px;"><i class="fa-solid fa-wand-magic-sparkles"></i> AI 智慧整理</div>`;
            
            const summaryContent = document.createElement("div");
            summaryContent.innerHTML = marked.parse(note.summary);
            summaryDiv.appendChild(summaryContent);
            
            noteDiv.appendChild(summaryDiv);
        } else {
            const sumBtn = document.createElement("button");
            sumBtn.className = "btn btn-secondary";
            sumBtn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> ✨ AI 智慧整理';
            sumBtn.style.cssText = "width: 100%; font-size: 12px; padding: 6px;";
            sumBtn.onclick = () => summarizeNote(note.id, sumBtn);
            noteDiv.appendChild(sumBtn);
        }
        
        const deleteBtn = document.createElement("button");
        deleteBtn.innerHTML = '<i class="fa-solid fa-trash"></i> 刪除';
        deleteBtn.style.cssText = "background: none; border: none; color: #ff4a5a; font-size: 11px; cursor: pointer; margin-top: 8px; text-decoration: underline;";
        deleteBtn.onclick = () => {
...
async function summarizeNote(noteId, btnElement) {
    const apiKey = document.getElementById("apiKey").value.trim() || localStorage.getItem("gemini_live_api_key");
    if (!apiKey) {
        alert("請先在左側面板輸入您的 Google AI Studio API Key 才能使用智慧整理功能！");
        return;
    }
    ...
    btnElement.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 正在整理中...';
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errData.detail || "摘要處理失敗");
        }

        const data = await response.json();
        notes[noteIndex].summary = data.summary;
        localStorage.setItem("live_stream_notes", JSON.stringify(notes));
        renderNotes();

    } catch (e) {
        alert("智慧整理發生錯誤: " + e.message);
...
document.addEventListener("DOMContentLoaded", () => {
    renderNotes();
    // 確保有 marked.js 用於渲染 Markdown，若無則動態引入
